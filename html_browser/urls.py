...
urlpatterns = [
    ...
    url(r'content/(?P<folderAndPathUrl>.+?)/$', content_view.ContentView.as_view(), name='content'),
    ...
    url(r'deleteImage/(?P<folderAndPathUrl>.+?)/$', base_view.DeleteImageView.as_view(), name='deleteImage'),
    url(r'deleteUser/.*', admin_views.DeleteUserView.as_view(), name='deleteUser'),
    url(r'download/(?P<folderAndPathUrl>.+?)/(?P<fileName>.+?)/$', base_view.DownloadView.as_view(), name='download'),
    url(r'download_zip/(?P<folderAndPathUrl>.+?)/$', base_view.DownloadZipView.as_view(), name='downloadZip'),
    ...
    url(r'getNextImage/(?P<folderAndPathUrl>.+?)/(?P<fileName>.+?)/$', base_view.GetNextImageView.as_view(), name='getNextImage'),
    ...
    url(r'image_view/(?P<folderAndPathUrl>.+?)/(?P<fileName>.+?)/$', base_view.ImageView.as_view(), name='imageView'),
    url(r'groupAdmin/.*', admin_views.GroupAdminView.as_view(), name='groupAdmin'),
    url(r'upload/(?P<folderAndPathUrl>.+?)/$', base_view.UploadView.as_view(), name='upload'),
    url(r'userAdmin/.*', admin_views.UserAdminView.as_view(), name='userAdmin'),
    url(r'thumb/', base_view.ThumbView.as_view(), name='thumb'),
    ]

for folder in Folder.objects.all():
    urlpatterns.append(url(r'__%s__/(?P<folderAndPathUrl>.+?)/$' % folder.name,
